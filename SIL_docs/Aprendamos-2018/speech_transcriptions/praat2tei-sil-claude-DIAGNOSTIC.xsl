...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:param name="input" as="xs:string" select="'Leccion_05.txt'"/>
  ...
  for each line tokenize the line by delimiter
   ...
   <xsl:variable name="teiHeader">
      <teiHeader>
         <fileDesc>
            <titleStmt>
               <title>TEI output for tab separated bilingual text files exported from file Leccion_01.wav</title>
               <respStmt>
   ...
   <xsl:variable name="praat-parsed" as="element()*">
      <xsl:for-each-group select="$lines-into-tabs" group-by="start">
         <xsl:for-each select="current-group()">
            <xsl:if test="normalize-space(tier) = ''">
               <xsl:message>DIAGNOSTIC: blank tier found -- start=[<xsl:value-of select="start"/>] text=[<xsl:value-of select="text"/>] end=[<xsl:value-of select="end"/>]</xsl:message>
            </xsl:if>
