            ...
            <table n="2">
                <xsl:for-each select="$theCollection">
                    ...
                    <xsl:for-each select="descendant::span[@type = 'phrase' and contains(@ana, '#POSS')]">
                        ...
                        <xsl:variable name="pron" select="substring-after(substring-after(@target, ' '), '#')"/><!-- add space! -->

                        <tr>
                            <td>
                                <xsl:value-of select="$currentCollection/descendant-or-self::seg[@xml:id = $orth]/w" separator="&#xA0;"/>
                            </td><!-- need space between N and particle!! -->
                            <td>
                                <xsl:value-of select="$currentCollection/descendant-or-self::seg[@xml:id = $pron]/w/c" separator="&#xA0;"/><!-- see if you can add separator after /w only!! -->
                            </td>
                            <td>
                        ...
                        <br/>
                    </xsl:for-each><!-- make version that can return full context of observation -->
                </xsl:for-each>
            </table>
        </html>
    </xsl:template>
